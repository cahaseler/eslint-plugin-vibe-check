    ...
    return {
      ...
      ExportNamedDeclaration(node) {
        if (!node.source) {
          ...
          node.specifiers.forEach(specifier => {
            ...
            // Check if this local name was imported and not used elsewhere
          ...
          node.specifiers.forEach(specifier => {
            ...
            for (const [importedLocal, importInfo] of imports.entries()) {
              if (importInfo.source === source && 
                  ...
                  !usedIdentifiers.has(importedLocal)) {
                context.report({
                  node: specifier,
                  messageId: "importExport",
                  data: {
      ...
      ExportDefaultDeclaration(node) {
        if (node.declaration && node.declaration.type === 'Identifier') {
          ...
          // Check if this identifier was imported and not used elsewhere
          if (imports.has(localName) && !usedIdentifiers.has(localName)) {
            const importInfo = imports.get(localName);
            context.report({
              node,
      ...
      ExportAllDeclaration(node) {
        ...
        // Check if we have any imports from the same source that are not used
        for (const [localName, importInfo] of imports.entries()) {
          if (importInfo.source === source && !usedIdentifiers.has(localName)) {
            context.report({
              node,
              messageId: "importExport",
              data: {
